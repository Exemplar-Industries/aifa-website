/*
 * AI Film Academy - Homepage proof and member reviews
 */
  ...
  return (
    <div className="flex gap-1" aria-label="Five-star review">
      {Array.from({ length: 5 }).map((_, index) => (
        <svg key={index} className="h-4 w-4" style={{ color: "var(--afa-red)" }} fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
  ...
  return (
    <section id="proof" className="relative overflow-hidden bg-[#0D0D0D] py-20 md:py-28 grain-overlay">
      <div className="container relative z-10">
        <div className="grid gap-8 border-b border-white/15 pb-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-end">
          <h2 className="max-w-4xl text-[clamp(3.25rem,7vw,6.4rem)] leading-[0.88] text-[#F5F5F0]">
            The proof is in <span style={{ color: "var(--afa-red)" }}>the work.</span>
          </h2>
          <p className="max-w-2xl text-[1.12rem] leading-8 text-white/90 md:text-[1.28rem]">
            Build work through focused training, creative practice, feedback, and a private community that keeps you moving forward.
          </p>
        </div>

        <div className="mt-10 grid gap-px border border-white/15 bg-white/15 md:grid-cols-3">
          {PROOF_POINTS.map(([number, label]) => (
            <article key={label} className="bg-[#111111] p-8 md:p-10">
              <p className="stat-number text-[clamp(4.2rem,7vw,6.5rem)]" style={{ color: "var(--afa-red)" }}>{number}</p>
              <p className="mt-3 text-[1.12rem] font-semibold leading-7 text-white/90 md:text-[1.22rem]">{label}</p>
            </article>
          ))}
        </div>
        ...
        <div className="mt-16 border-t border-white/15 pt-12">
          <div className="flex flex-wrap items-end justify-between gap-5">
            <h3 className="max-w-4xl text-[clamp(3rem,5vw,5.2rem)] leading-[0.9] text-white">
              What members are <span style={{ color: "var(--afa-red)" }}>saying.</span>
            </h3>
            <div className="flex items-center gap-3 text-[1rem] font-semibold text-white/90"><Stars /><span>5.0 on Google</span></div>
          </div>
